...
Original file is located at
    https://colab.research.google.com/drive/18sJ4q8qZFQFnsPGpFre4WHgtteF063-X
...
def double_resolution(image_path):
    # Load the image
    ...
    new_dimensions = (2 * width, 2 * height)
...
folder_path = '/content/Original'

# Output folder path to save the resized images
output_folder = '/content/Double_Resolution'
...
for filename in os.listdir(folder_path):
    ...
    if filename.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp')):
        ...
        resized_img = double_resolution(input_path)

        if resized_img is not None:
            # Save the resized image
            cv2.imwrite(output_path, resized_img)
            print('Saved resized image:', output_path)
...
# def test_color_150():
#     trans = FishEyeGenerator(150, [322,384])
#     img = cv2.imread(IMAGE_PATH)
...
#     print("Focal Length: 150")
#     cv2_imshow(img)
#     cv2_imshow(dst)
#     output_path = os.path.join("/content/Fisheye", "output_image_150.jpg")
...
def test_color_150():
    trans = FishEyeGenerator(150, [1280,1280])
    folder_path = "/content/drive/MyDrive/Final_Project/wires/DDOS/Val/Labels"
    output_folder = "/content/drive/MyDrive/Final_Project/wires/Fisheye/Val/Labels"
    ...
    for filename in os.listdir(folder_path):
        ...
        if filename.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp')):
            # Construct the input and output paths
            input_path = os.path.join(folder_path, filename)
            output_path = os.path.join(output_folder, 'Fisheye_150_' + filename)

            # You need to read each image file in the loop
            ...
            if dst is not None:
                # Save the resized image
                cv2.imwrite(output_path, dst)
                print('Saved Fisheye image:', output_path)
...
def test_color_300():
    trans = FishEyeGenerator(300, [1280,1280])
    folder_path = "/content/drive/MyDrive/Final_Project/wires/DDOS/Val/Labels"
    output_folder = "/content/drive/MyDrive/Final_Project/wires/Fisheye/Val/Labels"
    ...
    for filename in os.listdir(folder_path):
        ...
        if filename.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp')):
            # Construct the input and output paths
            input_path = os.path.join(folder_path, filename)
            output_path = os.path.join(output_folder, 'Fisheye_300_' + filename)
...
!zip -r Fisheye_Labels.zip Fisheye_Labels
...
def apply_fisheye(image_path):
    ...
    # Assuming some camera intrinsic parameters
    K = np.array([[800, 0, img.shape[1]/2], [0, 800, img.shape[0]/2], [0, 0, 1]])
    D = np.array([[0.8, 0.8, -0.8, 0.8]])  # Increasing distortion coefficients

    # Prepare a map for the fisheye transformation
    ...
    cv2_imshow(fisheye_img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

apply_fisheye('/content/0000035.jpg')
...
# Example usage:
# dpi = get_dpi('path_to_your_image.jpg')
# print(f"The image dpi is {dpi}")
...
# Usage:
img_path = '/content/0000029.jpg'
resized_img = double_resolution(img_path)
